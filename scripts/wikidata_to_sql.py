...
def main() -> int:
    ...
    for movie in payload.get("movies", []):
        ...
        statements.append(
            ...
            "title=excluded.title, language_name=excluded.language_name, release_date=excluded.release_date, "
    ...
    OUT.write_text("\n".join(statements) + "\n", encoding="utf-8")
    print(f"wrote {max(0, (len(statements)-2)//2)} movie upserts plus evidence to {OUT}")
    return 0

#!/usr/bin/env python3
"""Read-only production catalogue quality audit for Cinema & Series.

Input files are JSON produced by `wrangler d1 execute --remote --json`.
The audit never mutates production and deliberately keeps coverage dimensions
separate instead of inventing a composite quality score.
"""
...
def add_common_findings(
    ...
    for row in rows:
        ...
        if not norm(title):
            ...
            findings.append(finding("IDENTITY.QID_ONLY_TITLE", "S1", media_type, row, wikidata_qid=qid))
    ...
    for (_, year), group in groups.items():
        ...
        if len(group) > 1 and len(qids) > 1:
            findings.append(finding(
                "IDENTITY.POSSIBLE_DUPLICATE_TITLE_YEAR", "S2", media_type, group[0],
                year=year, entity_count=len(group), wikidata_qids=qids[:12],
            ))
    return findings
...
def analyze_catalogue(
    ...
    for row in movies:
        ...
        if not norm(title):
            ...
            findings.append(finding("IDENTITY.QID_ONLY_TITLE", "S1", "movie", row, wikidata_qid=qid))
        if not known(row.get("language_name")):
        ...
        if year_from_date(row.get("release_date")) is None:
            findings.append(finding(
                "METADATA.RELEASE_DATE_INVALID", "S1", "movie", row,
    ...
    for row in shows:
        ...
        if isinstance(first_year, int) and isinstance(last_year, int) and last_year < first_year:
            findings.append(finding(
                "SERIES.IMPOSSIBLE_AIR_YEAR_ORDER", "S0", "series", row,
                first_air_year=first_year, last_air_year=last_year,
            ))
    ...
    film_by_qid = {
        ...
        for row in films if valid_qid(row.get("wikidata_qid"))
    }
    show_by_qid = {
        str(row.get("wikidata_qid")).upper(): row
        for row in shows if valid_qid(row.get("wikidata_qid"))
    }
    for qid in sorted(set(film_by_qid) & set(show_by_qid)):
        movie, show = film_by_qid[qid], show_by_qid[qid]
        findings.append({
    ...
    findings.extend(duplicate_candidates(
        films,
        "movie",
        lambda row: year_from_date(row.get("release_date"))
        if row.get("_projection") == "movie" else row.get("release_year"),
    ))
    findings.extend(duplicate_candidates(shows, "series", lambda row: row.get("first_air_year")))

    film_count, show_count = len(films), len(shows)

    def film_year_known(row: Mapping[str, Any]) -> bool:
        return (
    ...
    def film_provenance_known(row: Mapping[str, Any]) -> bool:
        return (
            known(row.get("release_date_source"))
            if row.get("_projection") == "movie"
            else known(row.get("source_url"))
        )

    movie_coverage = {
        "identity_title": coverage(sum(
            1 for row in films
        ...
        "provenance_locator": coverage(sum(1 for row in films if film_provenance_known(row)), film_count),
    ...
    series_coverage = {
        ...
        "series_kind": coverage(sum(
            1 for row in shows if known(row.get("series_kind")) and norm(row.get("series_kind")) != "unknown"
        ), show_count),
        "lifecycle_status": coverage(sum(
            1 for row in shows
    ...
    language_lookup = {norm(language): language for language in INDIA_PRIORITY_LANGUAGES}
    for row in films:
        language = language_lookup.get(norm(row.get("language_name")))
        if language:
...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    for language, values in report["india_priority_languages"].items():
        lines.append(f"| {language} | {values['movies']:,} | {values['series']:,} | {values['total']:,} |")

...
class P1RebaselineOperationalContractTests(unittest.TestCase):
    ...
    def test_status_explicitly_marks_parent_writer_obsolete(self):
        self.assertIn("SHARD 3 COMPLETE", self.status)
        self.assertIn("physical shard 4", self.status)
        self.assertIn("35958174447:3", self.status)
        self.assertIn("35958228439", self.status)
        self.assertIn("45,683", self.status)
        self.assertIn("39,785", self.status)
        self.assertIn("4 / 15", self.status)
